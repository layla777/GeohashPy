...
GeohashNova は，Geohash の機能を Python によって包括的に実装したパッケージです．  
...
GeohashNova は PyPi に公開されており，以下のコマンドで簡単にインストールできます：

```sh
pip install goehash_nova
```
...
```python
gh.encode_with_lat_lng(91.0, -200.0)  # 緯度 91.0 → 89.0，経度 -200.0 → 160.0 に変換
...
```python
gh.geohash = 'ezs42p'  # 正常
gh.geohash = '123@abc"'  # エラー: 不正な文字 '@' を含む
gh.geohash = ''  # エラー: 空の文字列は無効
```
